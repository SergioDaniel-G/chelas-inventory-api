...
public class PrestadoControlador {
    ...
    public String guardarPrestados(@Valid @ModelAttribute("prestado") Prestado prestado,
                                   BindingResult result,
                                   RedirectAttributes flash,
                                   Model modelo) {
        if (result.hasErrors()) {
            modelo.addAttribute("prestados", prestado);
            modelo.addAttribute("titulo", "Registro de prestado");
            return "nuevo_prestados";
        }
    ...
    public String actualizarPrestados(@PathVariable Long id, @ModelAttribute("prestado") Prestado prestado,
        ...
        if (!prestadoExistente.getNombre().equals(prestado.getNombre())) {
            hayCambio = true;
        }
        if (!prestadoExistente.getCantidad().equals(prestado.getCantidad())) {
            hayCambio = true;
        }

        if (!prestadoExistente.getTotal().equals(prestado.getTotal())) {
            hayCambio = true;
        }
        if (!prestadoExistente.getDescripcionDelPedido().equals(prestado.getDescripcionDelPedido())) {
            hayCambio = true;
        }
        ...
        if (!hayCambio) {
            ...
            return "redirect:/prestados/editar/" + id;
        }
        ...
        prestado.setDescripcionDelPedido(prestado.getDescripcionDelPedido());
